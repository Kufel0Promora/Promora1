...
def verify_user_discord(user_id: str):
    """
    Sprawdza czy użytkownik jest na serwerze używając bezpośrednio API Discorda.
    NIE potrzebuje bota do tego!
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ Błąd API Discorda: {response.status_code} - {response.text}")
            return None
...
def register():
    ...
    print(f"🔍 Próba rejestracji: {user_id}")
    ...
    # WERYFIKACJA BEZPOŚREDNIO Z DISCORDEM!
    print(f"🔍 Sprawdzam użytkownika w Discordzie: {user_id}")
    result = verify_user_discord(user_id)
    print(f"📡 Wynik weryfikacji: {result}")
...
if __name__ == '__main__':
    print("🚀 API na http://localhost:5000")
    print(f"📡 Serwer ID: {GUILD_ID}")
